<template>
  <div v-if="!loading" class="overflow-x-auto p-4">
    <div class="grid grid-cols-3 gap-4">
      <h1 class="col-span-2 text-3xl font-bold mb-8">Medicamentos Faltantes</h1>
    </div>
    ...
    <table class="table table-compact w-full">
      ...
      <tbody v-for="(inventarios, index) in inventarios_faltantes" :key="index">
        <tr>
          <td>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      inventarios_faltantes: [] as Inventario[],
      filter: "",
      loading: true,
    };
  },
      ...
      try {
        ...
        this.filteredStock.forEach((inventario) => {
          if (inventario.piezas < 0) {
            this.inventarios_faltantes.push(inventario);
          }
        });
      } catch (error) {
        console.error(error);
